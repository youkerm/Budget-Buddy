...
$expenseSQL = "SELECT 'Expenses' AS 'category', COALESCE(CAST(SUM(amount)/(SUM(Revenue_Amount) + SUM(amount)) AS DECIMAL(9,2)), 0) AS 'measure' FROM `Expense`, `Revenue` WHERE Expense.AccountID = 6";
$revenueSQL = "SELECT 'Income' AS 'category', COALESCE(CAST(SUM(Revenue_Amount)/(SUM(Revenue_Amount) + SUM(amount)) AS DECIMAL(9,2)), 0) AS 'measure' FROM `Expense`, `Revenue` WHERE Expense.AccountID = 6";
